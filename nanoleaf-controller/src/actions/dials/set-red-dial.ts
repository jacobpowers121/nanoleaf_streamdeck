...
@action({ UUID: "com.jacob-powers.nanoleaf-controller.set-red-dial" })
export class SetRedDial extends SingletonAction<DialSettings> {
  ...
  override async onWillAppear(ev: WillAppearEvent<DialSettings>): Promise<void> {
    if (ev.action.isDial()) {
      ...
      globalRGBState.setColor("r", ev.payload.settings.value ?? 0);
      return ev.action.setFeedback(
        {
          "title": `Dial rotating ${this.tickValues[this.tickIndex]}`,
          "value": ev.payload.settings.value ?? 0,
          "icon": generateRGBPNG(ev.payload.settings.value ?? 0, 0, 0)
        }
      )
    }
  }
  ...
  override async onDialRotate(ev: DialRotateEvent<DialSettings>): Promise<void> {
    ...
    globalRGBState.setColor("r", value);
    await ev.action.setFeedback(
      {
        "title": `Dial rotating ${ev.payload.ticks * this.tickValues[this.tickIndex]}`,
        "value": value,
        "icon": generateRGBPNG(value, 0, 0)
      }
    );
    let sharedSettings = await ev.action.getSettings() as RGBSettings;
  }
