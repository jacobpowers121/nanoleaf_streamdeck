...
@action({ UUID: "com.jacob-powers.nanoleaf-controller.set-green-dial" })
export class SetGreenDial extends SingletonAction<DialSettings> {
  ...
  override async onWillAppear(ev: WillAppearEvent<DialSettings>): Promise<void> {
    if (ev.action.isDial()) {
      ...
      globalRGBState.setColor("g", ev.payload.settings.value ?? 0);
      return ev.action.setFeedback(
        {
          "title": `Dial rotating ${this.tickValues[this.tickIndex]}`,
          "value": ev.payload.settings.value ?? 0,
          "icon": generateRGBPNG(0, ev.payload.settings.value ?? 0, 0)
        }
      )
    }
  }
